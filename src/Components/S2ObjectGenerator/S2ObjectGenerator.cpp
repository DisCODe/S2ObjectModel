...
Eigen::Matrix4f S2ObjectGenerator::computeTransformationSAC(const pcl::PointCloud<PointXYZSIFT>::ConstPtr &cloud_src, const pcl::PointCloud<PointXYZSIFT>::ConstPtr &cloud_trg,
...
void S2ObjectGenerator::prepareInterface() {
    ...
    h_addViewToModel.setup(boost::bind(&S2ObjectGenerator::addViewToModel, this));
    ...
    h_addViewToModel_normals.setup(boost::bind(&S2ObjectGenerator::addViewToModel_normals, this));
...
//	CLOG(LINFO) << "SAC Transformation from current view cloud to cloud_merged: " << std::endl << current_trans;
    ...
    if (prop_ICP_alignment) {
        ...
        icp.setMaxCorrespondenceDistance (S2ObjectGenerator::ICP_max_correspondence_distance); //property
        // Set the maximum number of iterations (criterion 1)
        icp.setMaximumIterations (S2ObjectGenerator::ICP_max_iterations); // property
        // Set the transformation epsilon (criterion 2)
        icp.setTransformationEpsilon (S2ObjectGenerator::ICP_transformation_epsilon); //property
...
void S2ObjectGenerator::addViewToModel_normals() {

	CLOG(LTRACE) << "S2ObjectGenerator::addViewToModel_normals";
