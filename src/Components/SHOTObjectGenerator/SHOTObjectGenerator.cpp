...
Eigen::Matrix4f SHOTObjectGenerator::computeTransformationSAC(const pcl::PointCloud<PointXYZSHOT>::ConstPtr &cloud_src,
		const pcl::PointCloud<PointXYZSHOT>::ConstPtr &cloud_trg, const pcl::CorrespondencesConstPtr& correspondences,
...
void SHOTObjectGenerator::addViewToModel() {
	...
	CLOG(LINFO) << "determineCorrespondences";

	for (int i=0; i < correspondences->size(); ++i) {
		pcl::Correspondence corr = correspondences->at(i);
		std::cout << i << "\t" << corr.index_query << " - " << corr.index_match << " dist: " << corr.distance << "\n";
	}

	CLOG(LINFO) << "determineReciprocalCorrespondences";

	for (int i=0; i < correspondencesReciprocal->size(); ++i) {
		pcl::Correspondence corr = correspondencesReciprocal->at(i);
		std::cout << i << "\t" << corr.index_query << " - " << corr.index_match << " dist: " << corr.distance << "\n";
	}

	std::cout << "cloud_1: " << *cloud_1
			<< "\ncloud_2: " << *cloud_2
			<< "\nsrc: " << *src << "\ntgt: "<< *tgt << "\ncloud_shot: " << *cloud_shot << "\ncloud_shot_merged: " << cloud_shot_merged << "\n";
	//pcl::io::savePCDFileASCII ("correspondences.pcd", *correspondences);
	//pcl::io::savePCDFileASCII ("correspondencesReciprocal.pcd", *correspondencesReciprocal);

	pcl::io::savePCDFileASCII<pcl::PointXYZ>("cloud_1.pcd", *cloud_1);
	pcl::io::savePCDFileASCII<pcl::PointXYZ>("cloud_2.pcd", *cloud_2);
	pcl::io::savePCDFileASCII<pcl::PointXYZ> ("src.pcd", *src);
	pcl::io::savePCDFileASCII<pcl::PointXYZ> ("tgt.pcd", *tgt);
	pcl::io::savePCDFileASCII<PointXYZSHOT> ("cloud_shot.pcd", *cloud_shot);
	pcl::io::savePCDFileASCII<PointXYZSHOT> ("cloud_shot_merged_tgt.pcd", *cloud_shot_merged);
