...
class SHOTonlyXYZRepresentation: public pcl::DefaultFeatureRepresentation <PointXYZSHOT>
{
    ...
    virtual void
    copyToFloatArray (const PointXYZSHOT &p, float * out) const
    {
      out[0] = p.x;
      out[1] = p.y;
      out[2] = p.z;
...
void computeCorrespondences(const pcl::PointCloud<PointXYZSHOT>::ConstPtr &cloud_src, const pcl::PointCloud<PointXYZSHOT>::ConstPtr &cloud_trg, const pcl::CorrespondencesPtr& correspondences)
{
